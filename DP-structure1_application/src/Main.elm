        ...
        Nothing ->
            changeRoute url
                { page = Home
                    ...
                    , storage = Button.getModel (Button.init url)
                    , todo = ToDo.getModel (ToDo.init Nothing)
                    , button = Button.getModel (Button.init url)
                    , user = Nothing }
        Just token ->
                ({ page = Home
                    , key = key
...
retypeLogin : Model -> ( Login.Model, Cmd Login.Msg ) -> ( Model, Cmd Msg )
retypeLogin model ( login, cmd ) =
    case model.user of
        Nothing ->
            ({ model | page = Login }, Cmd.map Login_msg cmd )
        _ ->
            ({ model | page = Home }, Cmd.map Login_msg cmd )

retypeSignup : Model -> ( Signup.Model, Cmd Signup.Msg ) -> ( Model, Cmd Msg )
retypeSignup model ( signup, cmd ) =
    case model.user of
        Nothing ->
            ({ model | page = Signup }, Cmd.map Signup_msg cmd )
        _ ->
            ({ model | page = Home }, Cmd.map Signup_msg cmd )

retypeSigned : Model -> Signedup.Model -> ( Model, Cmd Msg )
retypeSigned model signedup =
    case model.user of
        Nothing ->
            ({ model | page = Signedup }, Cmd.none )
        _ ->
...
retypeTodo : Model -> ( ToDo.Model, Cmd ToDo.Msg ) -> ( Model, Cmd Msg )
retypeTodo model ( todo, cmd ) =
    case model.user of
        Nothing ->
            ({ model | page = ToDo }, Cmd.map ToDo_msg cmd )
        _ ->
            ({ model | page = ToDo }, Cmd.map ToDo_msg cmd )

retypeRituals : Model -> Access.Model -> ( Model, Cmd Msg )
retypeRituals model access =
    case model.user of
        Nothing ->
            ({ model | page = Access }, Cmd.none )
        _ ->
            ({ model | page = Ritual }, Cmd.none )

changeRoute : Url.Url -> Model -> ( Model, Cmd Msg )
changeRoute url model =
    let 
        parser = 
            ...
            [
                (Parser.s "login")
                  |> Parser.map ( retypeLogin model Login.init )
                , (Parser.s "signup")
                  |> Parser.map ( retypeSignup model Signup.init )
                , (Parser.s "signed")
                  |> Parser.map ( retypeSigned model Signedup.init )
                , (Parser.s "todo")
                  |> Parser.map ( retypeTodo model (ToDo.init Nothing) )
                , (Parser.s "rituals")
                  |> Parser.map ( retypeRituals model Access.init )
            ]
    in
        Maybe.withDefault ({ model | page = Home }, Cmd.none) (Parser.parse parser url)
